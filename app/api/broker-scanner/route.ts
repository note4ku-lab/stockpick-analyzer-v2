...
export async function POST(request:Request){
 ...
 try{
   ...
   const results=tickers.map((ticker:string)=>{const rows:BrokerRow[]=rowsByTicker[ticker]||[];const byCode=new Map(rows.map(row=>[String(row?.code??"").toUpperCase(),row]));const brokerStatus:Record<string,any>={};const targets=[...new Set([...buyBrokers,...sellBrokers])];for(const code of targets){const row=byCode.get(code);brokerStatus[code]={broker:code,side:buyBrokers.includes(code)?"BUY_TARGET":"SELL_TARGET",status:classify(row),metrics:summarize(row)}}
     ...
     const buyValue=buyBrokers.reduce((sum:number,code:string)=>sum+(brokerStatus[code]?.metrics?.buyValue||0),0);const sellValue=sellBrokers.reduce((sum:number,code:string)=>sum+(brokerStatus[code]?.metrics?.sellValue||0),0);const buyVolume=buyBrokers.reduce((sum:number,code:string)=>sum+(brokerStatus[code]?.metrics?.buyVolume||0),0);const sellVolume=sellBrokers.reduce((sum:number,code:string)=>sum+(brokerStatus[code]?.metrics?.sellVolume||0),0);const netValue=buyValue-sellValue,netVolume=buyVolume-sellVolume;const grossValue=buyValue+sellValue;const buyerDominance=grossValue>0?buyValue/grossValue:0;const brokerSignal=netValue>0?"AKUMULASI":netValue<0?"DISTRIBUSI":"NETRAL";const brokerStrength=Math.min(100,Math.round(Math.abs(netValue)/Math.max(grossValue,1)*100));const buyTargetText=buyBrokers.join(" + "),sellTargetText=sellBrokers.join(" + ");const intelligence={signal:brokerSignal,strength:brokerStrength,buyValue,sellValue,netValue,buyVolume,sellVolume,netVolume,buyerDominance,headline:brokerSignal==="AKUMULASI"?`${buyTargetText} dominan beli`:brokerSignal==="DISTRIBUSI"?`${sellTargetText} dominan jual`:"Buyer dan seller relatif seimbang",detail:brokerSignal==="AKUMULASI"?`${buyTargetText} menunjukkan dominasi akumulasi terhadap target SELL.`:brokerSignal==="DISTRIBUSI"?`${sellTargetText} menunjukkan dominasi distribusi terhadap target BUY.`:"Belum ada dominasi broker yang cukup jelas."};return {ticker,brokerCount:rows.length,score,verdict,matchCount,targetCount,buyMatches,sellMatches,brokers:brokerStatus,intelligence}}).sort((a:any,b:any)=>b.score-a.score||b.matchCount-a.matchCount);
