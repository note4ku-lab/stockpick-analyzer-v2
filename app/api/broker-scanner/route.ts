...
function classify(row:BrokerRow|undefined){if(!row)return "NOT_FOUND" as const;const buy=num(row.buy_value),sell=num(row.sell_value),bv=num(row.buy_volume),sv=num(row.sell_volume);if(buy===0&&sell===0)return "NO_ACTIVITY" as const;if(buy>sell&&bv>=sv)return "BUY" as const;if(sell>buy&&sv>=bv)return "SELL" as const;return "MIXED" as const}
function summarize(row:BrokerRow|undefined){if(!row)return null;const buyValue=num(row.buy_value),sellValue=num(row.sell_value),buyVolume=num(row.buy_volume),sellVolume=num(row.sell_volume);return {buyValue,sellValue,netValue:buyValue-sellValue,buyVolume,sellVolume,netVolume:buyVolume-sellVolume,buyAvg:num(row.buy_avg),sellAvg:num(row.sell_avg),buyFreq:num(row.buy_freq),sellFreq:num(row.sell_freq)}}
